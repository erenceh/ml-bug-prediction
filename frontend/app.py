  ...
  if st.button("Predict Priority"):
    if not title or not description:
      ...
      with st.spinner("Predicting..."):
        response = requests.post(f"{API_URL}/predict", json={"title": title, "description": description}, timeout=10)
        
        if response.status_code == 200:
          data = response.json()
          
          # Convert numeric labels to human-readable label using label_map
          ...
          fig = go.Figure(go.Indicator(
            mode="gauge+number",
            value=confidence,
            title={'text': f"Priority: {predicted_label} Confidence (%)"},
          ...
          if probs_dict:
            proba_df = pd.DataFrame(probs_dict.items(), columns=["Priority", "Probability"])
